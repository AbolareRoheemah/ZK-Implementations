...
//     fn generate_univariate_poly(&mut self, evals: &[F]) -> Vec<F> {
...
//     fn generate_proof(&mut self, evals: Vec<F>, claimed_sum: F) -> Proof<F> {
...
//             let univar_poly = self.generate_univariate_poly(&current_poly);
//             proof_array.push(univar_poly.clone()); // for verifier to be able to access it later
...
//         let proof_univars = &self.proof.univars;
